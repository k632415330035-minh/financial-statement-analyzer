...
export async function initOverview() {
  ...
  if (!overviewInited) {
    ...
    if (yearSelect) await renderBarChart(Number(yearSelect.value));
    await renderDonut();
    await renderGenderChart();
    await renderAgeChart();
    await renderPeriodChart();
...
async function renderKPIs() {
  ...
  gid('kpiHouseholds').textContent = kpis.households.toLocaleString('vi-VN');
  gid('kpiResidents').textContent = kpis.residents.toLocaleString('vi-VN');
  gid('kpiTempRes').textContent = kpis.tempResidence.toLocaleString('vi-VN');
  gid('kpiTempAbs').textContent = kpis.tempAbsence.toLocaleString('vi-VN');
}
...
async function initYearSelect() {
  ...
  await renderBarChart(Number(sel.value));
}
...
function initPeriodControlsStats() {
  ...
  document.querySelectorAll('[data-view="overview"] .chip').forEach(ch => ch.addEventListener('click', () => setRange(ch.dataset.range)));
  document.querySelector('.btnApplyStats')?.addEventListener('click', async () => {
    await renderPeriodChart();
    await renderPeriodTableStats();
  });
}
