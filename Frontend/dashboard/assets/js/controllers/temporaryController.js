...
function renderTempTable() {
  ...
  if (tbody) {
    ...
    tbody.innerHTML = slice.map((t, i) => {
      ...
      const statusColor = t.status === 'pending' ? '#f59e0b' : t.status === 'approved' ? '#22c55e' : '#ef4444';
      const statusText = t.status === 'pending' ? 'Chờ duyệt' : t.status === 'approved' ? 'Đã duyệt' : 'Bị từ chối';
      const actionBtns = t.status === 'pending'
        ? `<button class="btn" onclick="window.approveTempRecord('${t.id}')" style="font-size:12px;padding:6px 12px;margin-right:4px;">✓ Duyệt</button>
           <button class="btn" onclick="window.rejectTempRecord('${t.id}')" style="font-size:12px;padding:6px 12px;color:#ef4444;border-color:#ef4444;">✕ Từ chối</button>`
        : '';
      return `<tr>
